...
\leanmeta{\LH{L_ad7ca324}, \LH{L_0c281f80}, \LH{max\_leverage\_forces\_dof\_one}}
...
\leanmeta{Proved in \nolinkurl{Ssot} (\LH{oracle\_arbitrary})}
...
\leanmeta{\LH{dof\_eq\_srank}, \LH{ssot\_srank\_one}, \LH{incoherent\_srank\_gt\_one}}
...
\leanmeta{Proved in \LH{DecisionQuotient} (\LH{tractable\_bounded\_core}, \LH{sufficiency\_conp\_hard}), imported via \LH{BridgeToDQ.lean}}
...
\leanmeta{Combine Corollary 5.4 with Theorem 5.5; \LH{max\_coherence\_forces\_tractability}}
...
\begin{informal}
Architectures with more than one degree of freedom necessarily incur thermodynamic costs per decision cycle. Point~3 holds unconditionally: the energy bound follows from Landauer and the DOF~$=$~srank identity (\LH{dof\_eq\_srank}). Point~2 holds under the physical axioms of \cite{paper4_decision_quotient} (\LH{LP38}), which prove P~$\neq$~NP, hence P~$\neq$~coNP.
\end{informal}

Physical assumptions (point~3 only):
\begin{itemize}
\item Positive Landauer constant ($j_{\mathcal{M}} > 0$) — cf. Theorem \ref{cor:leverage-energy} (\LH{L_9c6279a3}, \LH{L_ebcd21af})
\item Landauer calibration ($k_B T \ln 2$ joules per bit)
\end{itemize}

\leanmeta{\LH{thermodynamic\_selection}, \LH{thermodynamic\_selection\_unconditional}, \LH{srank\_energy\_lower\_bound}}
...
\leanmeta{\LH{dof\_one\_iff\_max\_leverage}, \LH{oracle\_arbitrary}, \LH{dof\_eq\_srank}, \LH{max\_coherence\_forces\_tractability}, \LH{thermodynamic\_selection\_unconditional}}
...
\item \nolinkurl{Leverage/Foundations.lean}:
  \begin{itemize}
  \item \LH{L_ad7ca324}, \LH{L_0c281f80} (\LH{ssot\_max\_leverage}): DOF $= 1 \to$ max leverage
  \item \LH{max\_leverage\_forces\_dof\_one}: max leverage $\to$ DOF $= 1$
  \item \LH{dof\_one\_iff\_max\_leverage}: biconditional
  \end{itemize}
\item \nolinkurl{Leverage/BridgeToDQ.lean}:
  \begin{itemize}
  \item \LH{dof\_eq\_srank}: DOF $=$ srank
  \item \LH{ssot\_srank\_one}: DOF $= 1 \to$ srank $= 1$
  \item \LH{incoherent\_srank\_gt\_one}: DOF $> 1 \to$ srank $> 1$
  \item \LH{thermodynamic\_selection}: DOF $> 1 \to$ mandatory energy
  \item \LH{max\_coherence\_forces\_tractability}: max leverage $\to$ tractable
  \end{itemize}
\item \nolinkurl{DecisionQuotient/Tractability/StructuralRank.lean}: srank $= 1 \to$ P (\LH{tractable\_bounded\_core}), srank $> 1 \to$ coNP-hard (\LH{sufficiency\_conp\_hard})
\item \nolinkurl{DecisionQuotient/ThermodynamicLift.lean}: energy lower bounds under Landauer calibration (\LH{srank\_energy\_lower\_bound})
\end{itemize}

The cross-module dependency chain is live: \LH{Leverage} $\to$ \LH{DecisionQuotient} $\to$ Mathlib.
...
\leanmeta{\LH{england\_replication\_inequality}}

\begin{proof}
The gap is $(k-1) \cdot k_B \ln 2$. Since $k \leq 2^{k-1}$ (\LH{succ\_le\_two\_pow}), taking logs gives $\ln k \leq (k-1) \ln 2$, so the gap is $\geq k_B \ln k$.
\end{proof}
