...
import { buildAdj, findRoots, bfs, countComponents, isArtifact } from "./graph_utils";
...
// use shared graph utilities from graph_utils.js
...
function analyze(data) {
  ...
  return { total: data.nodes.length, edges: data.edges.length, components: countComponents(data.nodes, data.edges), maxDepth: maxD, groups: [...new Set(data.nodes.map(n => n.group).filter(Boolean))] };
}
          ...
          <div key={id} style={{display:"flex",gap:10}}>
            ...
            <div style={{paddingBottom:last?0:8}}>
              ...
              <div style={{fontSize:9,color:TH.dim}}>
                ...
                {last&&!isRoot&&<span style={{color:TH.bad,marginLeft:6}}>← requires rejecting this</span>}
              </div>
            </div>
          </div>);})}
      </div>
