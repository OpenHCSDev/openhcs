...
Restrict $F$ to the $k$ values in the clique. In the finite zero-error setting, any zero-error tag assignment is a proper coloring of the confusability graph, so a $k$-clique forces at least $k$ distinct tag values (\LH{GPH4}, \LH{IND1}). For the distribution-sensitive refinement, apply the Fano-style bound (Theorem~\ref{thm:fano-converse} in Section~\ref{sec:info-converse}) to conclude that vanishing error requires $I(F;S)\ge\log k - o(1)$. If $I(F;S)$ is strictly smaller than $\log k$, Fano's inequality implies a non-zero lower bound on error probability.
\end{proof}

\noindent The confusability graph provides a compact combinatorial certificate of necessary side-information: large cliques force large zero-error tag alphabets in the finite model (\LH{GPH4}, \LH{IND1}), and the Fano refinement turns the same obstruction into a mutual-information lower bound (\LH{CIA3}).
...
This timing constraint is the information-theoretic reason why derivation must be \emph{causal}--triggered by the act of defining the source, not by later access (\LH{SID1}, \LH{SID1}).
...
The two requirements--causal propagation and provenance observability--are independent. Neither implies the other (\LH{IND1}, \LH{INS2}).
