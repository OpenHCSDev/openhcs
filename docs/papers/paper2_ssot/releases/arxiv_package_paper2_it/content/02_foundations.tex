...
\end{theorem}
\leanmeta{\LH{ORA1}, \LH{ADV1}, \LH{TRI1}}
...
\end{corollary}
\leanmeta{\LH{COH3}, \LH{TRI1}, \LH{UNQ1}, \LH{MDL5}}

This is the information-theoretic foundation of optimal encoding under coherence constraints (\LH{COH1}, \LH{COH2}, \LH{COH3}).
...
Grünwald~\cite{gruenwald2007mdl} proves that MDL-optimal representations are unique under mild conditions. Theorem~\ref{thm:dof-optimal} establishes the analogous uniqueness for encoding systems under modification constraints: DOF = 1 is the unique coherence-guaranteeing rate (\LH{INS1}, \LH{INS2}).
...
\end{enumerate}
This uniqueness is forced by trichotomy (\LH{TRI1}): the three cases are exhaustive, and only DOF = 1 achieves coherence (\LH{UNQ1}). This is formalized as the Zero-Incoherence Capacity theorem (Theorem~\ref{thm:coherence-capacity}) in Section~\ref{sec:capacity}.
\end{theorem}
\leanmeta{\LH{INS1}}
...
We now formalize CAP and FLP inside the encoding model (\LH{CAP1}, \LH{FLP1}).
...
We now establish the central capacity result. This extends zero-error capacity theory~\cite{korner1973graphs,lovasz1979shannon} to interactive encoding systems with modification constraints (\LH{COH3}, \LH{CAP2}, \LH{CAP3}).
...
When an encoding system is incoherent, resolution requires external side information. We establish a tight bound on the required side information, connecting to Slepian-Wolf distributed source coding (\LH{CIA2}, \LH{SID1}, \LH{SID2}).
...
The set of derivation relations on an encoding system has algebraic structure. We characterize this structure and its computational implications (\LH{DER1}, \LH{AXM1}, \LH{CPL1}).
