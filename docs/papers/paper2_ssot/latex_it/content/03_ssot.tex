...
DOF = 1 is not a design guideline. It is the information-theoretically optimal rate guaranteeing coherence for facts encoded in systems with modification constraints (\LH{UNQ1}, \LH{UNQ1}, \LH{UNQ1}).
...
Our contribution is proving that SSOT is not a heuristic but an information-theoretic optimality condition (\LH{UNQ1}, \LH{UNQ1}, \LH{UNQ1}).
...
\end{proof}
See the formal Zero-Incoherence Capacity statement in Section~\ref{sec:capacity} (Theorem~\ref{thm:coherence-capacity}, \LH{UNQ1}). A concise proof sketch appears below; the full mechanized proof is provided in Supplement~A.
...
\begin{itemize}
\item \textbf{Converse (Fano):} To show that limited side information cannot enable vanishing-error recovery of $F$, apply the Fano-style bound (Theorem~\ref{thm:fano-converse}, \LH{UNQ1}). If the mutual information $I(F;S)$ provided by derived locations is $<\log K - o(1)$ for $K$ possible values, then any estimator has $P_e$ bounded away from zero. Thus perfect recoverability requires $I(F;S)\approx H(F)$, i.e. essentially $\log K$ bits.

\item \textbf{Confusability graph:} Lemma~\ref{lem:confusability-clique} converts combinatorial indistinguishability (cliques) into information lower bounds: in the finite zero-error model, a $k$-clique forces at least $k$ tag values (\LH{UNQ1}, \LH{UNQ1}); globally, the required tag alphabet is lower-bounded by both worst-case fiber size (\LH{UNQ1}) and the observation-alphabet occupancy floor (\LH{UNQ1}), equivalently $|\mathcal V|\le |\mathcal O|\cdot |\mathcal T|$ for any zero-error scheme (\LH{UNQ1}). The Fano refinement upgrades the same obstruction to a mutual-information lower bound for vanishing error (\LH{UNQ1}). This gives a language-independent certificate that side information is insufficient in environments with limited introspection.
