\section{Application: Programming-Language Instantiation}\label{sec:evaluation}
%==============================================================================

We instantiate Theorem~\ref{thm:ssot-iff} in the domain of programming languages. This section is intentionally application-level: the abstract IT results are complete without it, and the goal here is only to show how the realizability theorem specializes once a language's definition-time hooks and introspection capabilities are fixed.
...
\textbf{Interpretation.} DOF-1-complete languages can realize the abstract capacity-achieving structure for structural facts using language-native mechanisms. DOF-1-incomplete languages require external tooling (code generation, IDE refactoring) which operates outside language semantics and can be bypassed (cf.\ Theorem~\ref{thm:independence} on why external tools do not establish derivation).
