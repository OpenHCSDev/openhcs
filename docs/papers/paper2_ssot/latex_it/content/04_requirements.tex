...
Restrict $F$ to the $k$ values in the clique. In the finite zero-error setting, any zero-error tag assignment is a proper coloring of the confusability graph, so a $k$-clique forces at least $k$ distinct tag values (\LH{RED1}, \LH{RED1}). More generally, the required tag alphabet is bounded below by the worst-case observation-fiber size (\LH{RED1}), and therefore by the average fiber occupancy $\lfloor |\mathcal V|/|\mathcal O| \rfloor$ induced by observation alphabet size (\LH{RED1}); equivalently, any zero-error scheme must satisfy the global counting converse $|\mathcal V|\le |\mathcal O|\cdot |\mathcal T|$ (\LH{RED1}). For the distribution-sensitive refinement, apply the Fano-style bound (Theorem~\ref{thm:fano-converse} in Section~\ref{sec:info-converse}) to conclude that vanishing error requires $I(F;S)\ge\log k - o(1)$. If $I(F;S)$ is strictly smaller than $\log k$, Fano's inequality implies a non-zero lower bound on error probability.
\end{proof}

\noindent The confusability graph provides a compact combinatorial certificate of necessary side-information: large cliques force large zero-error tag alphabets in the finite model (\LH{RED1}, \LH{RED1}), the worst-case fiber bound captures the same obstruction globally (\LH{RED1}), a pigeonhole refinement gives an explicit alphabet-size floor (\LH{RED1}), and a global counting converse gives $|\mathcal V|\le |\mathcal O|\cdot |\mathcal T|$ for any zero-error scheme (\LH{RED1}). The Fano refinement turns this obstruction into a mutual-information lower bound (\LH{RED1}).
...
The two requirements--causal propagation and provenance observability--are independent. Neither implies the other (\LH{RED1}, \LH{RED1}).
