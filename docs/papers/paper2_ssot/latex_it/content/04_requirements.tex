...
We show that two encoder properties are necessary and sufficient (\LH{UNQ1}):
\begin{enumerate}
...
If the confusability graph for $F$ contains a clique of size $k$, then any zero-error side-information scheme for distinguishing all $k$ values requires at least $k$ distinct tags, equivalently at least $\log_2 k$ bits of side information.
\end{lemma}
\leanmeta{\LH{UNQ1}, \LH{UNQ1}, \LH{UNQ1}, \LH{UNQ1}, \LH{UNQ1}}

\begin{proof}[Proof sketch]
Restrict $F$ to the $k$ values in the clique. In the finite zero-error setting, any zero-error tag assignment is a proper coloring of the confusability graph, so a $k$-clique forces at least $k$ distinct tag values (\LH{UNQ1}, \LH{UNQ1}). More generally, the Lean observer/tag layer now proves three stronger structural facts: exact recovery is equivalent to injectivity of the combined observation-plus-tag map (\LH{OBS1}, \LH{OBS2}); each observation fiber requires at most as many states as available tags (\LH{OBS3}); and every clique therefore satisfies the direct alphabet bound $|\mathcal C|\le |\mathcal T|$ (\LH{OBS5}). Consequently, the required tag alphabet is bounded below by the worst-case observation-fiber size, and globally every zero-error scheme must satisfy $|\mathcal V|\le |\mathcal O|\cdot |\mathcal T|$ (\LH{OBS4}). Theorem~\ref{thm:fano-converse} packages the same obstruction as a finite counting converse: if only $2^L$ tags are available, then no zero-error resolver can distinguish more than $2^L$ ambiguous states.
\end{proof}

\noindent The confusability graph now has a fully mechanized finite zero-error interpretation: large cliques force large tag alphabets (\LH{OBS5}), nontrivial cliques are impossible for observation-only architectures (\LH{OBS6}), the worst-case fiber bound captures the same obstruction globally (\LH{OBS3}), and a global counting converse gives $|\mathcal V|\le |\mathcal O|\cdot |\mathcal T|$ for any zero-error scheme (\LH{OBS4}).
...
This timing constraint is the information-theoretic reason why derivation must be \emph{causal}--triggered by the act of defining the source, not by later access (\LH{UNQ1}, \LH{UNQ1}).
...
The two requirements--causal propagation and provenance observability--are independent. Neither implies the other (\LH{UNQ1}, \LH{UNQ1}). This section is included because it explains when the abstract IT optimum can actually be instantiated; it is not needed to state the main threshold theorem itself.
...
This independence means both requirements must be satisfied for DOF = 1 realizability (\LH{UNQ1}).
