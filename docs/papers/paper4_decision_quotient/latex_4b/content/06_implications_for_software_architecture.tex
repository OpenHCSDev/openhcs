...
\begin{proof}
This is the contrapositive of \LH{S2PH3}.
\end{proof}
...
In this paper's formal typing, over-specification is rational only under attempted competence failure: exact relevance competence is unavailable in the active regime \emph{after} an attempted exact procedure, and integrity forbids uncertified exclusion (Section~\ref{sec:interpretive-foundations}; Section~\ref{sec:engineering-justification}).\claimstamp{P\ref{prop:attempted-competence-matrix}}{AR}
Once exact competence is available in the active regime (Corollaries~\ref{cor:practice-bounded}--\ref{cor:practice-tree} together with Corollary~\ref{cor:exact-identifiability}), persistent over-specification is irrational for the same objective (verified total-cost minimization), because excluded coordinates can be certified irrelevant.\claimstamp{P\ref{prop:attempted-competence-matrix},T\ref{thm:tractable}}{TA}
\leanmeta{\LH{IC5},
\LH{IC11},
\LH{IC4},
\LH{L14},
...
\begin{proof}
By \LH{S2PH2}, sufficiency of size $\le k$ is equivalent to a relevance-cardinality bound $\le k$ in the Boolean-coordinate model.
\end{proof}
...
\begin{proof}
This is the bounded-regime branch of Theorem~\ref{thm:tractable}, mechanized as \LH{L14}.
\end{proof}
...
\begin{proof}
This is the separable-utility branch of Theorem~\ref{thm:tractable}, mechanized as \LH{L15}.
\end{proof}
...
\begin{proof}
This is the tree-factor branch of Theorem~\ref{thm:tractable}, mechanized as \LH{L16}.
\end{proof}
...
\begin{proof}
The strengthened reduction proves that non-tautological source formulas induce instances where every coordinate is relevant; this is mechanized as \LH{L1}.
\end{proof}
...
\begin{proof}
This is the mechanized theorem \LH{HD17}.
\end{proof}
...
\]
where $H_{\mathrm{baseline}}(P)$ corresponds to the Lean identifier
\texttt{intrinsicDOF}(P) in module \nolinkurl{HardnessDistribution}.
\leanmeta{\LH{HD15}}
\end{corollary}
