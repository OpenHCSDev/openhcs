...
/-- KL divergence is non-negative (Gibbs' inequality).
    ...
    This is the log-sum inequality - a fundamental result in information theory. -/
axiom kl_nonneg {Ω : Type*} [Fintype Ω]
    (P Q : Ω → ℝ) (hP : ∀ x, 0 ≤ P x) (hQ : ∀ x, 0 < Q x) :
    0 ≤ Finset.univ.sum (fun x => if P x > 0 then P x * Real.log (P x / Q x) else 0)

/-- Mutual information is non-negative: I(H;E) ≥ 0
    This follows from KL divergence being non-negative. -/
axiom mutual_info_nonneg : ∀ (mutualI : ℝ), True → 0 ≤ mutualI → 0 ≤ mutualI
