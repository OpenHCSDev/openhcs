...
Let a system have finite positive capacity $C > 0$. Let each information-acquisition event cost $\varepsilon > 0$. Then there exists a finite maximum event count $N_{\max}$ such that:
\[
  \varepsilon \cdot N \leq C \;\Rightarrow\; N \leq N_{\max}.
\]
...
\item[\textbf{TD.}] \textbf{Thermodynamics (Landauer).}
  ...
  Landauer established: $\varepsilon \geq k_B T \ln 2$ joules per bit.
  \cite{landauer1961irreversibility,bennett1982thermodynamics,berut2012experimental}
\end{enumerate}
...
\item \textbf{Energy (BA7 + TD):}
  Each bit operation costs $\geq k_B T \ln 2$ by \textbf{TD}.
  Minimum decision energy $= k \cdot k_B T \ln 2$.
\end{enumerate}
...
\end{proof}
The constant $k_B T \ln 2$ is not calibrated. It is the Landauer bound
applied to bit erasure, experimentally verified to $\pm 10\%$
\cite{berut2012experimental}. In the Lean model, \texttt{joulesPerBit} is
derived from \texttt{landauerJoulesPerBit} (ThermodynamicLift.lean), not
declared as a free parameter.
\LH{BA9}
\end{theorem}
\leanmeta{\LH{BA9}}
\claimstamp{T\ref{thm:physical-bridge-bundle}}{AR}
...
Among all decision problems, those with $\mathrm{srank} = 1$ have minimum
possible thermodynamic cost: $k_B T \ln 2$ per decision. This is the physical
ground state.

Problems with $\mathrm{srank} > 1$ are above the ground state by exactly
$(\mathrm{srank} - 1) \cdot k_B T \ln 2$ per decision cycle. This gap is
physically mandatory, not a consequence of implementation choice.
\LH{BA8}
\end{corollary}
\leanmeta{\LH{BA8}}
...
\item \textbf{Molecular level (IA9--IA12):} Two molecules in thermal equilibrium
  at temperature $T$ agree on $k_B T$. The Landauer bound applies universally.
...
invariant agreement, which requires rejecting quantum mechanics. No one will do this.
\LH{IA17}
\end{corollary}
\leanmeta{\LH{IA17}}
