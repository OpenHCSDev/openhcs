...
Proof sketch:
\begin{enumerate}
\item If $\phi$ preserves $\Opt$, then the quotient universal property already proved in the previous layer supplies the factorization $\pi = \psi \circ \phi$. If it does not preserve $\Opt$, then by definition there must exist $s,s'$ with $\phi(s)=\phi(s')$ but $\Opt(s)\neq\Opt(s')$. That witness is exactly the erased decision-relevant distinction.
