...
\begin{itemize}
\item \LH{sufficiency\_poly\_bounded\_actions}: Correctness of the algorithm.
\item \LH{bounded\_actions\_polynomial\_time}: Complexity bound $|S|^2 \cdot (1 + k^2)$.
\end{itemize}
...
\item \LH{separable\_optimal\_action\_constant}: Optimal actions are state-independent.
\item \LH{separable\_empty\_sufficient}: The empty set is sufficient.
\end{itemize}
...
\item \LH{tensor\_contraction\_tractable}: Axiom citing tensor network algorithms~\cite{kolda2009tensor,cichocki2016tensor}.
\item \LH{low\_rank\_sufficiency\_structure}: Reduction theorem for factored computation.
\item \LH{low\_rank\_tractability}: Tractability via factored sufficiency check.
\end{itemize}
...
\item \LH{tree\_dp\_tractable}: Axiom citing standard DP on trees.
\end{itemize}
...
\item \LH{treewidth\_le}: Axiom for treewidth (citing Bodlaender~\cite{bodlaender1993tourist}).
\item \LH{csp\_treewidth\_tractable}: Axiom citing CSP algorithms.
\item \LH{sufficiency\_reduces\_to\_interaction\_csp}: Paper-specific reduction theorem.
\item \LH{bounded\_treewidth\_tractability}: Tractability statement combining reduction and algorithm.
\end{itemize}
...
\item \LH{orbitType\_eq\_iff}: \textbf{(Core result)} Orbit types equal iff permutation exists.
\item \LH{symmetric\_optimalActions\_orbit\_invariant}: Optimal actions constant on orbits.
\item \LH{sufficiency\_reduces\_to\_cross\_orbit\_check}: Reduction to cross-orbit pairs.
\item \LH{symmetric\_sufficiency\_complexity\_bound}: $O\bigl(\binom{d+k-1}{k-1}^2\bigr)$ bound.
\end{itemize}
