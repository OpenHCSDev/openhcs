...
For functional set $F \subseteq \Omega$, the counting expression $-\log_2(|F|/|\Omega|)$ gives the bit cost, and a Landauer floor converts that bit count into a universal energy lower bound. The thermodynamic statement is therefore the counting statement plus an explicit empirical lower-bound constant; stronger empirical lower bounds only tighten it.
...
\textbf{EC1 (Temperature):} Entropy has energy cost. There exists $\varepsilon > 0$ such that each irreversible bit operation costs at least $\varepsilon$ joules, with $\varepsilon \geq k_B T \ln 2$. Landauer gives the universal floor; constrained implementations may require strictly larger per-bit dissipation.
\end{definition}
...
\item Each transition costs at least the Landauer floor $k_B T \ln 2$. Therefore $E \geq \mathrm{srank} \cdot k_B T \ln 2$ as a universal lower bound.
