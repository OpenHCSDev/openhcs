...
The thermodynamic lift in Section~\ref{sec:dichotomy} converts bit-operation lower bounds into energy/carbon lower bounds via linear conversion constants. In this artifact, those conversion consequences are mechanized with explicit premises through Propositions~\ref{prop:thermo-lift}, \ref{prop:thermo-hardness-bundle}, \ref{prop:thermo-mandatory-cost}, and \ref{prop:thermo-conservation-additive}. Landauer's principle and reversible-computation context~\cite{landauer1961irreversibility,bennett2003notes} define a minimum floor for irreversible bit erasure, not a generally tight expression for the actual energetic cost of constrained computation. The core stochastic-thermodynamics review literature~\cite{vandenbroeck2015ensemble} provides the entropy-production framework underlying that distinction. Recent work by Wolpert and collaborators makes the gap explicit: process constraints such as periodic driving, modularity, finite-time operation, stopping-time effects, and input mismatch generate unavoidable entropy production above the Landauer floor~\cite{wolpert2024stochastic,manzano2024absolute,yadav2026minimal}. Our formal model now represents that distinction directly through an explicit floor-plus-overhead interface: \LH{WC1} states the Landauer-floor-plus-overhead lower bound, \LH{WC2} proves continued dominance of the Landauer floor, \LH{WC3} gives strict separation when the declared constrained-process overhead is positive, \LH{WC4} proves monotone tightening of the induced energy lower bound, and \LH{WC5} lifts that stronger premise through the existing physical grounding bundle. This makes the Wolpert refinement a theorem-level interface rather than a comment-level interpretation.\claimstamp{P\ref{prop:thermo-lift},\ref{prop:thermo-hardness-bundle},\ref{prop:thermo-mandatory-cost},\ref{prop:thermo-conservation-additive}}{AR}\leanmeta{\LH{CC65},\LH{CC66},\LH{CC67},\LH{CC68},\LH{CC64},\LH{WC1},\LH{WC2},\LH{WC3},\LH{WC4},\LH{WC5}.}
