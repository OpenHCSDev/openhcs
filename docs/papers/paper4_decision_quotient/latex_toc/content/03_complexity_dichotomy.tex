...
Physical reading: the separation is an access-and-budget separation for the same decision semantics. Explicit access assumes full boundary observability; succinct access assumes compressed structure that may require exponential expansion to certify universally; query access assumes bounded measurement interfaces.\claimstamp{T\ref{thm:dichotomy};P\ref{prop:query-regime-obstruction},\ref{prop:physical-claim-transport}}{E,S+ETH,Qf,AR}
...
Let $C$ denote a complexity coordinate with $C = b_{\mathrm{LB}}$ from Proposition~\ref{prop:query-regime-obstruction}. Under the declared conversion model with $\lambda > 0$, the thermodynamic duality
\[
dU \ge \lambda\,dC
\]
...
Decision circuit defs & [AR] & Mechanized & \LH{IE1}--\LH{IE17}, \LH{CC1}--\LH{CC12} \\
Structural asymmetry & [AR,H=cost-growth] & Mechanized & \LH{SR1}--\LH{SR5} \\
Gap energy / DQ & [AR,H=cost-growth] & Mechanized & \LH{GE1}--\LH{GE9}, \LH{DQ1}--\LH{DQ8} \\
Self-erosion & [Q\_fin,H=cost-growth] & Mechanized & \LH{SE1}--\LH{SE6} \\
Channel = regime & [E,Q,S,AR] & Mechanized & \LH{CH1}--\LH{CH6} \\
Investment/conservation & [Q\_fin,H=cost-growth] & Mechanized & \LH{IV1}--\LH{IV6} \\
Atomic circuits & [Q\_fin,AR] & Mechanized & \LH{AC1}--\LH{AC11} \\
Information access & [E,Q,S,AR] & Mechanized & \LH{IA1}--\LH{IA6} \\
Dimensional complexity & [E,Q,S,AR,H=tractable-structured] & Mechanized & \LH{DC1}--\LH{DC15} \\
Discrete spacetime & [AR] & Mechanized & \LH{DS1}--\LH{DS6} \\
Neukart--Vinokur & [AR,H=cost-growth] & Derived & \LH{CC67}--\LH{CC69} \\
\bottomrule
\end{tabular}
\end{center}
...
\leanmeta{\LH{DE1},\LH{DE2},\LH{DE3},\LH{DE4}.}
\end{proposition}
...
\leanmeta{\LH{GE4},\LH{GE5}.}
\end{corollary}

\begin{definition}[Decision Quotient from Gap]\label{def:dq-from-gap}
\claimstamp{D\ref{def:dq-from-gap}}{AR,H=cost-growth}
The \emph{decision quotient} measures how much current state predicts future state:
\[
...
\leanmeta{\LH{DQ1},\LH{DQ2},\LH{DQ3}.}
\end{definition}

\begin{theorem}[Decision Quotient as Physical Primitive]\label{thm:dq-physical}
\claimstamp{T\ref{thm:dq-physical}}{AR,H=cost-growth}
The decision quotient $\mathrm{DQ} = 1 - \text{GapEnergy}/\text{TotalEnergy}$ is a physical quantity with:
\begin{enumerate}
  \item $\mathrm{DQ} \in [0, 1]$ \quad \leanmeta{\LH{BF3}}
  \item $\mathrm{DQ} = 0 \Leftrightarrow$ maximum gap (no information) \quad \leanmeta{\LH{BF5}}
  \item $\mathrm{DQ} = 1 \Leftrightarrow$ zero gap (deterministic) \quad \leanmeta{\LH{BF4}}
  \item $\mathrm{DQ} + \text{Gap}/\text{Total} = 1$ (complementarity) \quad \leanmeta{\LH{BF6}}
\end{enumerate}
\leanmeta{\LH{BF1}, \LH{BF2}.}
\end{theorem}
...
\begin{enumerate}
  \item \textbf{No free information}: $\mathrm{DQ}(U(\text{prior}, e)) \le \mathrm{DQ}(\text{prior}) + I(e)$ \quad \leanmeta{\LH{BF7}}
  \item \textbf{Landauer bound}: energy cost $\ge k_BT \ln 2 \times$ bits updated \quad \leanmeta{\LH{BF8}}
  \item \textbf{Integrity preservation}: can't corrupt verified bits for free \quad \leanmeta{\LH{BF9}}
\end{enumerate}
Then: $U$ is admissible $\Leftrightarrow$ $U$ is Bayesian updating.
\leanmeta{\LH{BF10}, \LH{BF11}, \LH{BF12}.}
\end{theorem}
...
The uniqueness corollary \LH{BF13} shows any two admissible rules must agree.

\textbf{Machine-checked foundation.} The core inequality underlying this result---that Bayesian updating uniquely minimizes cross-entropy---is proved in Lean 4 with zero axioms and zero \texttt{sorry} statements. The proof derives Gibbs' inequality ($\mathrm{KL}(p\|q) \geq 0$) from the fundamental inequality $\log x \leq x - 1$, then shows $\mathrm{CE}(p,q) = H(p) + \mathrm{KL}(p\|q)$, yielding $\mathrm{CE}(p,p) \leq \mathrm{CE}(p,q)$ with equality iff $p = q$.\footnote{Source: \url{https://github.com/OpenHCSDev/openhcs/blob/main/docs/papers/paper4_decision_quotient/proofs/DecisionQuotient/BayesOptimalityProof.lean}}
\end{proof}

\begin{corollary}[Thermodynamic Decision Quotient]\label{cor:thermo-dq}
\claimstamp{C\ref{cor:thermo-dq}}{AR,H=cost-growth}
In energy terms: $\mathrm{DQ} = 1 - \text{GapEnergy}/\text{TotalEnergy}$. The decision quotient has a thermodynamic interpretation: it is the fraction of total uncertainty energy that is decision-relevant.
\leanmeta{\LH{DQ7},\LH{DQ8}.}
\end{corollary}
...
\leanmeta{\LH{IE3},\LH{IE4},\LH{IE5}.}
\end{corollary}

\subsubsection{Foundational Derivation of Bayesian Reasoning}
...
\begin{enumerate}
  \item \textbf{Fraction $\Rightarrow$ Probability}: For finite $\Omega$, define $P(A) = |A|/|\Omega|$. This satisfies the Kolmogorov axioms: nonnegativity \LH{FN2}, normalization \LH{FN3}, and finite additivity \LH{FN4}.
  \item \textbf{Conditional $\Rightarrow$ Bayes}: Define $P(H|E) = P(H \cap E)/P(E)$. Then $P(H|E) = P(E|H) P(H) / P(E)$ follows in two lines by commutativity of intersection \LH{FN6}.
  \item \textbf{KL $\geq 0$ $\Rightarrow$ Entropy Contraction}: From Gibbs' inequality $D_{\text{KL}}(P \| Q) \geq 0$ \LH{FN7}, the chain rule $I(H;E) = H(H) - H(H|E)$ yields $H(H|E) \leq H(H)$ \LH{FN8}. Conditioning cannot increase entropy.
  \item \textbf{Normalization $\Rightarrow$ DQ}: Define $\text{DQ} = I(H;E)/H(H) = 1 - H(H|E)/H(H)$ \LH{FN9}. This is the fraction of prior uncertainty eliminated by observing $E$, bounded in $[0,1]$ \LH{FN11}.
\end{enumerate}
The complete chain is mechanized in a single theorem \LH{FN14}.
...
\begin{itemize}
  \item \textbf{No free information}: DQ cannot increase without evidence (2nd law) \LH{BF7}.
  \item \textbf{Landauer bound}: DQ increase costs $\geq k_B T \ln 2$ per bit \LH{BF8}.
  \item \textbf{Integrity preservation}: verified bits cannot be corrupted for free \LH{BF9}.
\end{itemize}
Under these constraints, Bayesian updating is the \emph{unique} admissible rule \LH{BF12}. Any two admissible rules must agree \LH{BF13}. This is ``Bayes from physics''---the thermodynamic constraints uniquely determine the correct update rule.
...
\leanmeta{\LH{IE14},\LH{IE15},\LH{IE16}.}
...
\subsubsection{Atomic Circuits: Matter as Agent}\label{sec:atomic-circuits}
...
Changing an atom's electronic configuration is a decision event (Definition~\ref{def:decision-event}). Orbital transitions are state transitions~\cite{sakurai2017modern}. Atoms with orbital transitions are nonstationary decision circuits---agents.
...
An atom in its ground state with no transitions is passive: it is not an agent. Agents are defined by state transitions, not by substrate.
\leanmeta{\LH{AC6}.}
\end{corollary}
...
\begin{definition}[Agent as Matter Moving Matter]\label{def:matter-agent}
\claimstamp{D\ref{def:matter-agent}}{Q\_fin,AR}
An \emph{agent} is matter that pays energy to change other matter's state. A decision circuit is a physical system that implements state transitions. A nonstationary decision circuit is an agent.
...
\leanmeta{\LH{SE1},\LH{SE2},\LH{SE3},\LH{SE4}.}
\end{theorem}
...
A \emph{regime} (Definition~\ref{def:regime-model}) corresponds to a \emph{channel} in the Shannon sense: a constraint on what information can flow. The explicit-state regime E, query-access regime Q, and succinct regime S define distinct channels with distinct capacity bounds.
\leanmeta{\LH{CH1}.}
\end{definition}
