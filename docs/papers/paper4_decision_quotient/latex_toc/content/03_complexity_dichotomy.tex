...
\begin{proof}
The theorem \LH{L2} constructs, for any query set of cardinality $<2^n$, an unqueried hidden state $s_0$ and a yes/no instance pair that agree on all queried values but disagree on $\emptyset$-sufficiency. The auxiliary bound \LH{L6} ensures that fewer than $2^n$ value-entry queries cannot cover all states, so the indistinguishability argument applies in the worst case.
\end{proof}
...
Neukart--Vinokur & [AR,H=cost-growth] & Derived & \LH{TL1}--\LH{TL7} \\
\bottomrule
\end{tabular}
\end{center}
...
\begin{theorem}[Decision Quotient Derived from Bayes]\label{thm:dq-bayes}
\claimstamp{T\ref{thm:dq-bayes}}{AR,H=cost-growth}
The decision quotient has the following properties:
\begin{enumerate}
  \item $\mathrm{DQ} \in [0, 1]$.
  \item $\mathrm{DQ} = 0$: Current integrity provides no information about future (maximum gap, pure noise).
  \item $\mathrm{DQ} = 1$: Current integrity perfectly determines future (zero gap, deterministic).
  \item $\mathrm{DQ} + \text{Gap}/\text{Total} = 1$: Gap and decision quotient are complementary.
\end{enumerate}
\leanmeta{\LH{DQ4},\LH{DQ5},\LH{DQ6}.}
\end{theorem}

\begin{proof}
By definition of mutual information and conditional entropy. The decision quotient is the fraction of future uncertainty resolved by knowing the present. The gap is the fraction remaining.
\end{proof}
...
\begin{enumerate}
\item Bounded actions $\to$ P \quad \leanmeta{\LH{DC2}}
\item Separable utility $\to$ P \quad \leanmeta{\LH{DC3}}
\item Low tensor rank $\to$ P \quad \leanmeta{\LH{DC4}}
\item Tree structure $\to$ P \quad \leanmeta{\LH{DC5}}
\item Bounded treewidth $\to$ P \quad \leanmeta{\LH{DC6}}
\item Coordinate symmetry $\to$ P \quad \leanmeta{\LH{DC7}}
\end{enumerate}
\leanmeta{\LH{DC13}.}
\end{theorem}
...
\leanmeta{\LH{DC10},\LH{DC11},\LH{DC12},\LH{DC14}.}
\end{theorem}
