...
\item The cost paid (substrate-dependent $\kappa$)
\end{itemize}

\begin{claim}{Bayesian structure detection}{th:bayesian-learning}
Agent maintains posterior $P(C \mid I_1, \ldots, I_k)$ over structure classes. Update rule: standard Bayesian update with likelihood $P(I_k \mid C)$.
...
How many instances must the agent solve before it can identify the structure class with probability $\geq 1 - \delta$ and error $\leq \varepsilon$?
