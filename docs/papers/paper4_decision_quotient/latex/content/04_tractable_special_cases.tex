...
\begin{itemize}
\item \LH{L9}: Correctness of the algorithm.
\item \LH{L1}: Complexity bound $|S|^2 \cdot (1 + k^2)$.
\end{itemize}
...
\item \LH{L7}: The empty set is sufficient.
\end{itemize}
...
\item \LH{L5}: Tractability via factored sufficiency check.
\end{itemize}
...
\item \LH{L15}: Axiom citing standard DP on trees.
\end{itemize}
...
\item \LH{L3}: Axiom citing CSP algorithms.
\item \LH{L11}: Paper-specific reduction theorem.
...
\item \LH{L13}: $O\bigl(\binom{d+k-1}{k-1}^2\bigr)$ bound.
\end{itemize}
