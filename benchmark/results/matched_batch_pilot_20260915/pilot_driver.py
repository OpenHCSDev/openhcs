...
(case,) = [
    c
    for c in load_comparison_cases(
        PROJECT / "benchmark/manifests/official30_portable_axis1.json"
    )
    if c.name == "cp_tutorial_translocation_final"
]
...
wells = tuple(
    sorted(
        {
            source_component_metadata_value(m, AllComponents.WELL)
            for m in prepared.materialization.source_metadata.values()
        }
    )
)[:8]
...
global_config = GlobalPipelineConfig(
    ...
    path_planning_config=PathPlanningConfig(
        global_output_folder=ROOT / "candidate", output_dir_suffix="_matched_pilot"
    ),
...
if (ROOT / "native_report.json").exists():
    ...
    native_completed = subprocess.run(
        [
            str(PROJECT / ".venv-cellprofiler39/bin/python"),
            str(PROJECT / "benchmark/native_cellprofiler_batch_worker.py"),
            str(ROOT / "native_request.json"),
        ],
        cwd=PROJECT,
        env=os.environ.copy(),
        capture_output=True,
...
port = DataControlPortPairAuthority.acquire(
    OPENHCS_ZMQ_CONFIG, transport_mode=TransportMode.TCP
).data_port
events = []
...
submission = OpenHCSExecutionSubmission(
    ...
    pipeline_document=PipelineDocumentAuthority.from_values(
        pipeline_config=pipeline_config, pipeline_steps=prepared.pipeline_steps
    ),
    global_config=global_config,
...
try:
    with client:
        ...
        for repetition in range(-1, 3):
            ...
            compiled = ExecutionSubmissionResponse.from_wire(
                client.submit_compile(submission)
            )
            artifact_id = compiled.require_execution_id("Pilot compilation")
            ExecutionWaitResult.from_wire(
                client.wait_for_completion(artifact_id, poll_interval=0.05)
            ).require_complete("Pilot compilation")
            ...
            ExecutionWaitResult.from_wire(
                client.wait_for_completion(execution_id, poll_interval=0.05)
            ).require_complete("Pilot execution")
            completed = time.perf_counter()
            completed_wall = time.time()
            (ROOT / ("events_" + str(repetition) + ".json")).write_text(
            ...
            axis_started = min(
                ...
                if event.phase is ProgressPhase.AXIS_STARTED
            )
            probe_receipt = {
                ...
                "first_axis_through_completion_observed_seconds": completed_wall
                - axis_started,
                ...
                "output_files": [
                    str(path) for path in observation.exports.output_files
                ],
                "execution_failures": observation.execution_failures(),
                "validation_failures": runtime_artifact_execution_failures(
                    observation.expectation, observation.observation()
                ),
            }
            (ROOT / "candidate_probe.json").write_text(
                json.dumps(probe_receipt, indent=2)
            )
...
report = {
    ...
    "scoped_dirty_diff_sha256": hashlib.sha256(
    ...
    "candidate_environment": {
        ...
        "openhcs_version": openhcs.__version__,
