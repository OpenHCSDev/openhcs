...
def declump_intensity(
    binary: np.ndarray, intensity: np.ndarray, min_distance: int = 7
) -> np.ndarray:
    """
...
def identify_primary_objects(
    ...
    min_dist = (
        min_allowed_distance if min_allowed_distance else max(1, min_diameter // 2)
    )
    ...
    for i, slice_2d in enumerate(image):
        ...
        if threshold_method == "minimum_cross_entropy":
            thresh_val = (
                minimum_cross_entropy_threshold(smoothed) * threshold_correction
            )
        else:
        ...
        measurements.append(
            NucleiMeasurement(
                ...
                mean_intensity=(
                    float(np.mean([p.mean_intensity for p in props])) if props else 0.0
                ),
            )
        )
