    ...
    def identify_primary_objects_cupy(
        ...
        for i, slice_2d in enumerate(image):
            ...
            coords = peak_local_max(
                cp.asnumpy(weighted), min_distance=min_dist, labels=cp.asnumpy(binary)
            )
            ...
            if discard_outside_diameter:
                ...
                valid_labels = [
                    p.label for p in props if min_area <= p.area <= max_area
                ]
            ...
            measurements.append(
                NucleiMeasurement(
                    ...
                    mean_intensity=(
                        float(np.mean([p.mean_intensity for p in props]))
                        if props
                        else 0.0
                    ),
                )
