...
try:
    ...
    def preprocess_gpu(
        ...
        for slice_2d in image:
            ...
            # Gaussian smoothing
            if gaussian_sigma > 0:
                gpu_slice = cle.gaussian_blur(
                    gpu_slice, sigma_x=gaussian_sigma, sigma_y=gaussian_sigma
                )

            # Optional median filter
            if median_size > 0:
                gpu_slice = cle.median(
                    gpu_slice, radius_x=median_size // 2, radius_y=median_size // 2
                )
