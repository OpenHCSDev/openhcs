...
class PatternFileService:
        ...
        def _sync_load_pattern(path: Path) -> Union[List, Dict]:
            ...
            try:
                ...
                if not isinstance(pattern, (list, dict)):
                    raise ValueError(
                        f"Invalid pattern type: {type(pattern)}. Expected list or dict."
                    )
    ...
    async def save_pattern_to_file(
        self, pattern: Union[List, Dict], file_path: Path
    ) -> None:
        """
        ...
        def _sync_save_pattern(pattern_data: Union[List, Dict], path: Path) -> None:
            ...
            if not isinstance(pattern_data, (list, dict)):
                raise ValueError(
                    f"Invalid pattern type: {type(pattern_data)}. Expected list or dict."
                )
    ...
    async def validate_pattern_file(
        self, file_path: Path
        ...
        def _sync_backup_file(original_path: Path) -> Path:
            ...
            if backup_path.exists():
                ...
                backup_path = original_path.with_suffix(
                    f"{original_path.suffix}.backup.{timestamp}"
                )
