...
def _detect_cells_voronoi_otsu(
    gpu_image: cle.Array, slice_idx: int, params: Dict[str, Any]
) -> CellCountResult:
    """
    ...
    blurred = cle.gaussian_blur(
        gpu_image, sigma_x=params["gaussian_sigma"], sigma_y=params["gaussian_sigma"]
    )
    ...
    voronoi_labels = cle.remove_small_labels(
        voronoi_labels, minimum_size=params["min_cell_area"]
    )
    voronoi_labels = cle.remove_large_labels(
        voronoi_labels, maximum_size=params["max_cell_area"]
    )
    ...
    if "label" in stats_dict and len(stats_dict["label"]) > 0:
        positions = [
            (float(x), float(y))
            for x, y in zip(stats_dict["centroid_x"], stats_dict["centroid_y"])
        ]
...
def _detect_cells_threshold(
    ...
    if "label" in stats_dict and len(stats_dict["label"]) > 0:
        ...
        confidences = [
            float(intensity / max_intensity_cpu) for intensity in intensities
        ]
...
def _create_labeled_mask(gpu_image: cle.Array, result: CellCountResult) -> np.ndarray:
    ...
    for i, (x, y) in enumerate(result.cell_positions):
        ...
        valid_mask = (
            (rr >= 0) & (rr < image_shape[0]) & (cc >= 0) & (cc < image_shape[1])
        )
