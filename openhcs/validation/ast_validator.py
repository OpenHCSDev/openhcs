...
ERROR_VFS_BOUNDARY = "VFS Boundary violation: {0}"
ERROR_MEMORY_TYPE = "Memory type violation: {0}"
...
class ASTValidator(ast.NodeVisitor, metaclass=AutoRegisterMeta):
    ...
    def validators_for(
        ...
        selected_kinds = (
            ...
            if kinds is None
            else tuple(ValidationKind(kind) for kind in kinds)
        )
        return tuple(cls.__registry__[kind] for kind in selected_kinds)

    def add_violation(
        self, node: ast.AST, violation_type: ValidationKind | str, message: str
    ) -> None:
...
class BackendParameterValidator(ASTValidator):
    ...
    def visit_Call(self, node: ast.Call) -> None:
        ...
        if node.func.attr in filemanager_methods:
            ...
            backend_arg = node.args[
                -1
            ]  # Backend should be the last positional argument
            if isinstance(backend_arg, ast.Constant) and not isinstance(
...
class VFSBoundaryValidator(ASTValidator):
    ...
    def __init__(self, file_path: str):
        ...
        self.in_test_module = "/tests/" in file_path or os.path.basename(
            file_path
        ).startswith("test_")
    ...
    def visit_FunctionDef(self, node: ast.FunctionDef) -> None:
        ...
        for decorator in node.decorator_list:
            if isinstance(decorator, ast.Name) and decorator.id in memory_decorators:
                ...
                and decorator.func.id in memory_decorators
            ):
                has_memory_decorator = True
                break
