...
- Before operating a live desktop workflow, call {agent_capabilities.ui_list_windows.name} and resolve visible modal top-level windows first. Inspect each dialog through {agent_capabilities.ui_get_widget_tree.name} and invoke its declared button through {agent_capabilities.ui_invoke_widget_action.name}; acknowledge completed operations, cancel unsolicited update/restart prompts, and never use desktop-coordinate automation or leave a blocking dialog hidden behind the analysis viewer.
