...
- Begin from declarations and evidence. Search the example corpus for the closest biological operation, retrieve its complete generated OpenHCS Python, and describe each candidate through the live registry. Reuse declared structure and callable docstrings; do not dispatch on function-name strings or create parallel catalogues.
...
- Treat registration as a separate branch. Build the declared reference, calculate one position set, and retain its typed artifact. Restart assembly from declared sources with `input_source=PIPELINE_START` when needed, reuse that same position set by consuming its artifact, and assemble raw channel stacks or stacks whose analytical normalisation was explicitly fitted across all sites. Never recalculate positions by channel, display window, or result.
- Preserve correspondence across every QC view. Inspect the raw mosaics at several percentile clipping windows, including the full range and progressively stronger low/high pairs, while keeping placement coordinates, crop, scale, and result overlays identical. Record each percentile pair and its computed intensity bounds. Compare channel histograms, clipped fractions, and segmentation coverage across tile interiors, joins, quadrants, and complete fields; report per-region object or traced-signal density so seams, field-dependent thresholds, and spatial sensitivity drift are measurable rather than merely visible.
- Display normalisation changes presentation only; analytical preprocessing requires an explicit compiled FunctionStep. Analytically normalised data retain parameters, source/group identity, assembled SITE axis, position artifact, and output artifact provenance; a similar-looking viewer layer is not that record.
- Validate both intensity extremes and the requested result. Inspect representative dim structures, bright structures, background, saturated pixels, and strong unassigned residuals in native coordinates. For masks, sample supported foreground, missed-signal components, and foreground unsupported by source signal; report false-negative residual coverage, unsupported-mask coverage, object splits/merges, and boundary agreement when a reference exists. For traces or spatial graphs, sample intensity along each path and in a surrounding background band, then inspect low-support segments, high-signal residuals, discontinuities, gaps, endpoints, branches, crossings, and ownership continuity. Never infer support from display colour or a nonzero mask alone.
- {ImageAnalysisQaPolicy.repair_guidance()}
- Choose validation evidence before tuning. Freeze a blinded, spatially distributed representative set containing dim signal, bright signal, sparse background, dense structures, tile boundaries, and known controls; do not replace difficult fields after seeing results. Tune only on the declared development subset. Perturb each sensitive parameter around the candidate value while holding sources, coordinates, preprocessing, display windows, and controls fixed, and report whether object count, covered signal, path length, branches, endpoints, and control ordering remain stable. Score held-out fields once after freezing the pipeline.
- Treat each diagnostic rerun as one falsifiable experiment. Record the observed failure, one explicit hypothesis, one declaration-owned semantic change, the complete pipeline identity, compile result, bounded execution time, peak memory, and the same-coordinate visual and quantitative evidence. Preserve every rejected attempt instead of overwriting it; multiple simultaneous changes make repair quality unscoreable.
- When exact catalogue search cannot express the required operation, follow the custom-function authoring context and register one typed callable through the existing authority. Before using it, verify that the same reflected signature and defaults appear in function detail, generated code, the parameter form, and the MCP contract. A local script or unregistered callable is not an OpenHCS pipeline result.
- Keep the complete analytical operation inside the OpenHCS declaration and managed runtime. Do not preprocess scientific inputs in an external script, inject text or clicks through X11/desktop automation, or manipulate a viewer behind the MCP surface. Use source bindings, function patterns, artifacts, compiler evidence, and managed viewer commands so the pipeline remains reproducible and every observation retains its owner.
- Escalate rather than declare success when the raw/result comparison finds coherent missed signal, unsupported foreground or traces, split/merged objects that alter ownership, unexplained tile/quadrant drift, unstable controls, strong parameter sensitivity, or disagreement between artifacts, ROIs, and measurement rows. Preserve the current attempt and provenance, constrain the next run to representative evidence, revise the smallest declaration-owned parameter or function boundary that explains the failure, and repeat the same coordinates and metrics. Ask the domain expert when signal identity or biological acceptability cannot be decided from image evidence; do not hide uncertainty inside an aggregate.
- Reconcile image/label or graph artifacts with schema-bearing measurements and ROIs, including identities and cardinalities. Record enough counts and coordinates to reproduce the sampled checks. A completed execution, plausible aggregate, smooth mosaic, or non-empty layer proves neither spatially uniform segmentation nor scientific completeness.
- Use the corpus as a pattern source, not an authority. Reuse source-bound preprocessing, registration-reference branches that share one position artifact, diagnostic intermediates followed by an interpretable final artifact, and bounded validation before plate-scale execution. Current bindings, function contracts, compiler plan, runtime routes, and artifact provenance remain authoritative."""
...
- Review one current execution in raw-evidence order: confirm execution and route identity plus resolved source order; inspect bounded source/output arrays and statistics; inspect typed label IDs and ROI payloads; reconcile schema-bearing per-object measurement rows with those objects; only then interpret the visualization with the biologist. Aggregate viewer indices are not necessarily a route-local semantic coordinate because routes can have distinct component domains and axis offsets. Before accepting a screenshot, read the target route's typed component values and positional indices, navigate through that route, re-read viewer state, and reject the capture when its active route or component values do not match the routed payload identity.
...
- `materialize_runtime_artifacts` enables a persistent target; it does not override an artifact output's own materialization declaration. Inspect the compiled artifact plan before promising files: an automatically streamed object-label ROI projection can be streaming-only. When durable label arrays are required, use a declared catalogue projection or an explicit typed artifact materialization that participates in persistence, then verify the reported paths exist before freezing the pipeline.
