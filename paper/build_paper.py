...
class SlasRetainedFigures(Preparation):
    def resolve(self, root: Path, figures: tuple[Path, ...], observations: InputObservations) -> tuple[str, ...]:
        for path in (root / "build_docx_from_markdown.py", root / "requirements-build.txt",
                     *sorted((root / "figures").glob("build_slas*.py"))):
            observations.read(path)
        coverage = ReceiptCoverage.discover(root / "figures/slas", figures, root.parent, observations)
        return coverage.validate(observations)

    def refresh(self, root: Path) -> None:
        raise RuntimeError("SLAS generators depend on historical/external analysis and live UI captures. No safe automatic refresh is declared; follow paper/README.md separately.")
