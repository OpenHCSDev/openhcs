...
SUMMARY = (
    ROOT
    / "benchmark/results/labmeeting_20260513/official30_well_throughput/data/single_process_summary.csv"
)
...
def build() -> None:
    ...
    for example in EXAMPLES:
        ...
        for index, (step, recovered) in enumerate(
            ...
            for original, returned in zip(original_items, restored_items, strict=True):
                if (
                    original.contract.module_name != returned.contract.module_name
                    or original.contract.function_name
                    != returned.contract.function_name
                    or not semantic_values_equal(
                        original.kwargs_dict, returned.kwargs_dict
                    )
                ):
                    raise ValueError(
                        f"Function round-trip mismatch: {example.title}, step {index}"
                    )
                sources.add(
                    Path(
                        inspect.getfile(
                            original.contract.resolve_canonical_raw_callable()
                        )
                    )
                )
            names = ", ".join(
                dict.fromkeys(item.contract.function_name for item in original_items)
            )
        ...
        lines.extend(
            f"| {number} | {name} | `{function}` | {count} |"
            for number, name, function, count in rows
        )
        lines.append("")
    ...
    (OUTPUT / "complex_cellprofiler_workflows_provenance.json").write_text(
