    ...
    with McpDevClient() as client:
        ...
        def call(name, **arguments):
            result = client.execute(
                ["call", name, "--arguments", json.dumps(arguments), "--json"]
            )
            receipt["events"].append(
                {"tool": name, "arguments": arguments, "response": result.payload}
            )
        ...
        def get_document(document_id):
            return call(
                "openhcs_ui_get_code_document", document_id=document_id, clean=False
            )
        ...
        def widget_value(expected):
            tree = call(
                ...
                window_id=args.scope,
                max_depth=18,
                max_nodes=280,
                maximum_text_length=180,
            )
            matches = [
                node
                for node in tree["actionable_widgets"]
        ...
        edited = before["source"].replace(
            "'high_percentile': 99.8", "'high_percentile': 99.6"
        )
        assert parameter_values(edited, parameter) == [99.6]
        apply_source(before, edited)
        assert parameter_values(get_document(child_document)["source"], parameter) == [
            99.6
        ]
        widget_value(99.6)
        ...
        restored = call(
            ...
            object_state_scope_id=child_scope,
            field_path=parameter,
            value=99.8,
            window_id=args.scope,
        )
        assert restored["mutated"], restored
        assert parameter_values(get_document(document_id)["source"], parameter) == [
            99.8
        ]
        widget_value(99.8)
        ...
        call(
            ...
            window_id=args.scope,
            path_id=code_buttons[0]["path_id"],
            action_kind="button",
        )
        for _ in range(30):
