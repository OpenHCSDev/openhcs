...
RESULTS = (
    ROOT / "mcp_outputs/website-agent-demo/candidate-20260804-13/outputs/plate_openhcs"
)
SELECTED_NEURON = 2
...
def read_shapes(path):
    with ZipFile(path) as archive:
        ...
        return tuple(
            (name, info, ImagejRoi.frombytes(archive.read(name)).coordinates())
...
def build():
    ...
    graph_path = (
        RESULTS
        / "images_results/1_s001_w1_z001_t001_neurite_morphology_step1.graph.roi.zip"
    )
    table_path = (
        RESULTS
        / "images_results/1_site-1_z_index-1_timepoint-1_neurite_outgrowth_cells_step1_details.csv"
    )
    ...
    if not roi_binding.source == table_binding.source == graph_binding.source:
        raise ValueError(
            "The displayed outputs no longer share one declared object source"
        )
    ...
    if (
        row_ids != roi_ids
        or not graph_ids <= row_ids
        or SELECTED_NEURON not in graph_ids
    ):
        raise ValueError(
            "Saved rows, shapes and graph paths do not have matching neuron IDs"
        )
    for _, info, coordinates in shapes:
        ...
        if not np.all((coordinates >= -0.5) & (coordinates <= 799.5)):
            raise ValueError("ROI coordinates lie outside the declared image")
    summary = corrected["result_summary"]
    if (
        len(row_ids) != summary["neurons"]
        or sum(float(row["total_outgrowth_um"]) for row in rows)
        != summary["total_outgrowth_pixels"]
        or sum(int(row["branches"]) for row in rows) != summary["branches"]
        or len(graph_shapes) != summary["spatial_graph_path_count"]
    ):
        raise ValueError(
            "Archived result counts differ from the corrected public recapture"
        )

    sheet = FigureSheet(
        "outputs_and_inspection", "Keep images, objects and measurements connected", 9.2
    )
    for path in (
        record_path,
        image_path,
    ...
    sheet.box(
        ...
        7,
        "Named results",
        "available to later processing steps",
        color=TEAL,
    )
    ...
    sheet.box(
        ...
        "TIFF images · ROI ZIPs · CSV tables",
        color=BLUE,
    )
    sheet.box(
        54,
        ...
        7.5,
        "View images + objects",
        "napari / Fiji (live streaming)",
        color=TEAL,
    )
    sheet.text(
        50,
        47.8,
        "Saving and live viewing can be enabled independently.",
        ha="center",
        size=10,
        color=MUTED,
    )
    ...
    selected_coordinates = np.concatenate(
        [
            coords
            for _, info, coords in shapes
            if info[subject_id_key] == SELECTED_NEURON
        ]
    )
    margin = 25
    left, top = np.maximum(
        0, np.floor(selected_coordinates.min(axis=0)).astype(int) - margin
    )
    right, bottom = np.minimum(
        image.shape[::-1],
        np.ceil(selected_coordinates.max(axis=0)).astype(int) + margin,
    )
    axis = sheet.figure.add_axes((0.05, 0.155, 0.40, 0.255))
    ...
    for _, info, coordinates in shapes:
        ...
        axis.plot(
            coordinates[:, 0],
            coordinates[:, 1],
            color=ORANGE if selected else "#8ba7ba",
            linewidth=1.3 if selected else 0.45,
            alpha=1 if selected else 0.55,
        )
    axis.set(xlim=(left, right), ylim=(bottom, top))
    axis.set_axis_off()
    axis.set_title(
        f"Saved review image · neuron {SELECTED_NEURON}", fontsize=10, color=PURPLE
    )
    sheet.text(
        72, 40.2, "Saved per-neuron measurements", size=10, ha="center", color=ORANGE
    )
    table_axis = sheet.figure.add_axes((0.50, 0.185, 0.44, 0.19))
    ...
    sheet.text(
        72, 16, "Same source image + neuron ID", size=10, color=PURPLE, ha="center"
    )
    sheet.text(
        5,
        12.5,
        "Public NeuronCyto II field · retained corrected run · selected detail replotted",
        size=9.5,
        color=MUTED,
    )
    ...
    for x, title, color in (
        (5, "Feature-table row", BLUE),
        (37, "Object outline", PURPLE),
    ...
    receipt["retained_execution"] = {
        "kind": "post_qa_corrected_recapture",
        "openhcs_version": corrected["openhcs_version"],
    ...
    receipt["identity_validation"] = {
        ...
        "measurement_and_roi_ids": sorted(row_ids),
        "selected_neuron": SELECTED_NEURON,
        "selected_roi_members": [
            name for name, info, _ in shapes if info[subject_id_key] == SELECTED_NEURON
        ],
        "selected_graph_members": [
            name
            for name, info, _ in graph_shapes
            if info[subject_id_key] == SELECTED_NEURON
        ],
    }
    ...
    receipt["declared_output_formats"] = {
        spec.name: [
            option.primary_output_suffix for option in spec.materialization.outputs
        ]
        for spec in (
            UNIFIED_NEURONS_OUTPUT,
            NEURITE_CELLS_OUTPUT,
