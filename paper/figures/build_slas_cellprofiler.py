...
def build():
    ...
    for module, step in pairs:
        if module.name != step.name:
            raise ValueError(
                "Reassess the figure layout: module-to-step grouping changed"
            )
        invocations = tuple(normalize_function_pattern(step.func).iter_items())
        declared = CellProfilerModule.require_module(
            module.name
    ...
    for original, recovered in zip(steps, restored.pipeline_steps, strict=True):
        for before, after in zip(
            ...
            normalize_function_pattern(recovered.func).iter_items(),
            strict=True,
        ):
            if (
                before.contract.function_name != after.contract.function_name
                ...
                raise ValueError(
                    "Generated Python did not preserve imported function parameters"
                )
    document_path.write_text(document_source)
    sheet = FigureSheet(
        "cellprofiler_translation",
        "A CellProfiler workflow becomes editable processing steps",
        10.1,
    )
    for source in (
        SOURCE,
    ...
    aliases = ", ".join(
        binding.alias for binding in config.source_bindings_config.bindings
    )
    sheet.box(
        47,
        79.3,
    ...
    for index, (module, step) in enumerate(pairs, 1):
        ...
        if index % 2:
            sheet.axis.add_patch(
                Rectangle((3, y - 1.45), 93, 2.9, facecolor=PALE, edgecolor="none")
            )
        scope = FunctionStepExecutionScope.require_uniform(
            item.contract for item in items
        )
        color = ORANGE if scope is FunctionStepExecutionScope.PLATE else TEAL
        sheet.text(
            5, y, f"{module.module_num:02d}  {module.name}", size=9.5, va="center"
        )
        sheet.arrow((38, y), (44, y), color=color)
        sheet.text(
            46, y, f"{index:02d}", size=9.5, color=color, va="center", weight="bold"
        )
        sheet.text(
            50, y, names[0], family="monospace", size=9.6, va="center", color=INK
        )
        if len(items) > 1:
            sheet.text(
                94.5,
        ...
        for item in items:
            sheet.source(
                Path(inspect.getfile(item.contract.resolve_canonical_raw_callable()))
            )
        ...
        records.append(
            {
                ...
                "calls": [
                    {
                        "module": item.contract.module_name,
                        "function": item.contract.function_name,
                        "kwargs_python": repr(item.kwargs_dict),
                    }
                    for item in items
                ],
            }
    ...
    shape_module, shape_step = next(
        pair for pair in pairs if pair[0].name == "MeasureObjectSizeShape"
    )
    ...
    varied_keys = tuple(
        key
        for key, value in shape_items[0].kwargs
    ...
    sheet.text(
        5,
        26.1,
        function_name + "(..., " + object_key + "=...)",
        family="monospace",
        size=9,
        color=TEAL,
    )
    for call_number, (item, x) in enumerate(
        zip(shape_items, (5, 37, 69), strict=True), 1
    ):
        sheet.box(
            x,
            17.4,
            26,
            6.6,
            repr(item.kwargs_dict[object_key]),
            f"Call {call_number}",
            color=PURPLE,
        )
    sheet.text(
        50,
        15.0,
        "Same function, distinct named objects, ordered within one step",
        size=10,
        color=MUTED,
        ha="center",
    )
    ...
    mask_module, mask_step = next(
        pair for pair in pairs if pair[0].name == "MaskObjects"
    )
    ...
    sheet.box(
        ...
        27,
        5.7,
        mask_item.contract.function_name,
        "invert_mask=True",
        color=TEAL,
    )
    ...
    receipt["source_modules"] = [
        {"number": module.module_num, "name": module.name} for module in modules
    ]
