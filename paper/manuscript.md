...
OpenHCS throughput was measured in repeated-input runs over all 30 workflows. With four repeated-image assignments queued per worker, median throughput across workflows was 1.79, 2.67 and 2.96 completed assignments per execution second with two, three and four workers, respectively (Figure 5A). Every assignment completed. Workers were reused within each run under the output settings in Methods. Supplementary Data 3 retains the individual runs.
...
\(A) Completed repeated-image assignments per execution second with four assignments queued per worker. (B) Peak memory with four workers and increasing assignments per worker. Each point is one of the 30 workflows, shaded boxes span the interquartile range, vertical lines span the observed range and black lines mark medians. Every assignment completed. Execution follows compilation, and workers are reused within each run. The recorded configuration disables default saving of named results and return of detailed worker records, and requests removal of unused steps whose outputs are not saved. Throughput uses a logarithmic axis. Supplementary Figure 8 shows the same observations by workflow; Supplementary Figure 6 presents the separate historical single-sample timings.
