...
**Author-information draft:** Confirm the complete author list and order, departmental or institute affiliations, and corresponding-author designation before submission.
...
OpenHCS represents an analysis as an ordered list of configured processing steps shared by the graphical interface, Python and MCP clients (Figure 1). A registered function supplies the parameter names, types, defaults and documentation used to build its controls and catalog description. Editable Python records the selected functions and their configured values. This connects the interfaces at the workflow definition: a scientist can open an agent-authored pipeline, change a parameter in a control or in code, and run the revised analysis. Imported CellProfiler workflows and custom Python functions use the same model.
...
We examined the shared workflow through a recorded GUI/Python editing round trip, CellProfiler reference-output comparisons, and agent-operated analysis of a public neurite-outgrowth image. The CellProfiler workflows span cellular morphology, Cell Painting, translocation, wound healing, tracking, yeast screening and worm phenotyping. Separate execution measurements assess completed-sample throughput and memory use as worker count and queue depth change. These evaluations connect interactive workflow reuse, agent operation and repeated-sample execution.
...
Separate runs measured throughput and peak memory by assigning the same source images to multiple well identifiers, creating repeated analysis work (Table 2). Queue depth specifies how many assignments were supplied per worker. Each condition has one recorded run per workflow, with workers reusing loaded libraries and initialized backends within that execution.
...
Throughput uses execution time after initialization and compilation. The recorded configuration disables default saving of named results and return of detailed worker records, and requests removal of unused steps whose outputs are not saved. Supplementary Data 3 identifies these settings and individual runs. Measurements cover CPU execution on local or explicitly mounted image sources; GPU and cloud or network-storage performance were not measured.
...
The agent inspected the images' organization and constructed a two-step pipeline: normalization followed by a registered neurite morphology and topology function (Figure 3). Of 140 attempted MCP calls, one was rejected for an invalid operation name; 30 others returned an error from the requested operation. Those errors covered code validation, request formats, discovery, file queries and UI operations (Supplementary Data 4). Code-document validation rejected incorrect preset imports and configuration values before execution; the agent revised the document until validation succeeded, then compiled and ran it in the desktop session. The saved record reports 609 s from start to completion, including authoring, corrections, execution and agent output checks; pipeline execution took 16.5 s. Outputs included neuron and nuclear labels, per-neuron measurements, neurite paths, ROI files and an SWC morphology file. The pipeline remained editable in Python and the graphical interface.

The final napari view displayed enhanced neuronal signal, unified neuron labels and neurite paths, with a feature table linking paths to measurements. Structured viewer checks found all nine expected outputs present with no missing or duplicate coordinates. The original analysis reported nine neurons, ten nuclei and 25 graph paths. Subsequent visual review identified a crossover classified as branching. A later correction to the registered neurite graph-extraction strategy, demonstrated in OpenHCS 0.7.14, reported the same neuron and nucleus counts, six branches instead of eight, and 24 graph paths instead of 25. Supplementary Data 4 separates the original and later result summaries. The example records agent-operated construction, execution and inspection followed by scientist-led review and method refinement.

### Figure 3. Agent-operated analysis of a public neurite-outgrowth image

![Original inputs and recorded viewer inspection.](figures/slas/figure3_agent_workflow.png){width=6in}

(A, B) Original 800 x 800-pixel neuronal/neurite and soma/nuclear images from NeuronCyto II field 1 [@NeuronCytoII], displayed linearly over their unsigned 8-bit range without spatial cropping. The boxes identify Step 1 and Step 2 using names and functions from the agent's saved pipeline. (C) Full frame at 600 s of the original OpenHCS 0.7.13 unattended-run recording, showing outlines, ROI entries and the feature table. Labelled rectangles locate enlarged details: (D) neuron and neurite outlines and (E) three path rows with neuron labels and distance columns. Both are unchanged pixel crops from C. Native column names are preserved; the run record does not establish physical calibration for `_um` columns. This is the original result before the crossover correction described in Results.

### Imported CellProfiler workflows match the compared reference outputs

All 30 imported workflows compiled and executed successfully in the OpenHCS 0.8.5 CI run. Twenty-five had retained native reference values: 21 contained CSV measurements, three contained SQLite measurements and CellProfiler Analyst properties, and one contained an illumination array. All 25 passed their selected value comparisons. The other five pipelines declared no file-export modules and completed without reference-value comparisons. Their intermediate computations include object combination, nuclear segmentation and illumination correction (Supplementary Data 1).

Image comparisons passed for two of the 25 compared workflows: the illumination-array example and the translocation example, whose overlay was compared alongside its SQLite measurements. Figure 4 shows how named images, objects and operations are retained in the imported Comet Assay.
...
OpenHCS throughput was measured in repeated-input runs over all 30 workflows. With four repeated-image assignments queued per worker, median throughput across workflows was 1.79, 2.67 and 2.96 completed assignments per execution second with two, three and four workers, respectively (Figure 5A). Each run completed all assigned images. Workers were reused within each run under the output settings in Methods. Supplementary Data 3 retains the individual runs.
...
\(A) Completed repeated-image assignments per execution second with four assignments queued per worker. (B) Peak memory with four workers and increasing assignments per worker. Both panels include all 30 workflows; every run completed its assigned wells. Thin lines identify workflows and thick lines show medians. Execution follows compilation, and workers are reused within each run. The recorded configuration disables default saving of named results and return of detailed worker records, and requests removal of unused steps whose outputs are not saved. Throughput uses a logarithmic axis. Supplementary Figure 6 presents the separate historical single-sample timings.
...
The evaluations cover complementary parts of this workflow. Release CI checks newly executed OpenHCS analyses against retained native CellProfiler values for the selected inputs, settings and output types. The single-field agent demonstration records completion under one prompt and client/model configuration, with scientific interpretation assessed by subsequent visual review rather than a ground-truth accuracy study. Its crossover correction shows why segmentation and tracing need review even after execution succeeds. The separate throughput measurements describe OpenHCS scaling under the recorded output policy; matched native CellProfiler throughput remains to be measured.
...
1. **CellProfiler workflow comparison:** OpenHCS 0.8.5 CI observations, reference inventory and run provenance, followed by separate historical timing records. The unresolved native wound-healing time is excluded from manuscript timing comparisons.
...
3. **Worker and memory measurements:** measured execution and memory by workflow, worker count and well count, including completion status.
...
The benchmark manifest maps workflows to pinned revisions of their source collections. The [release CI evidence](supplementary/ci_official30_085/README.md) preserves per-workflow observations and run metadata from the OpenHCS 0.8.5 test, with checksums and links to its exact source revision and hosted job. Historical performance records and separately versioned agent demonstrations are indexed alongside it.
