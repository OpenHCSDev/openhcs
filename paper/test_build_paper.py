...
def test_local_declaration_has_one_source_authority_and_pair():
    ...
    assert tuple(document.role for document in PAPER.documents) == (DocumentRole.MANUSCRIPT, DocumentRole.SUPPLEMENT)
    assert PAPER.documents[0].sources == (Path('manuscript.md'),)
    assert 'sole editable source' in (PAPER.root / 'openhcs_nature_methods_draft.md').read_text()


def test_ordinary_figure_validation_reuses_receipts(tmp_path):
    _, checks = resolve_inputs(PAPER, MarkdownDocumentBuilder(), BuildLog(tmp_path / 'validate.log'), InputObservations())
...
def test_compatibility_command_delegates_and_resolves_old_source(tmp_path):
    ...
    result = subprocess.run([sys.executable, str(PAPER.root / 'build_docx_from_markdown.py'),
                             str(PAPER.root / 'openhcs_nature_methods_draft.md'), str(output)],
                            cwd=tmp_path, capture_output=True, text=True)
    assert result.returncode == 0, result.stderr
    with zipfile.ZipFile(output) as archive:
        assert 'shared microscopy workflows' in archive.read('word/document.xml').decode()
